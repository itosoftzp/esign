function SimpleStackException(msg) {
  ...
  PDFAnnotate.prototype.enableRectangle = function () {
    ...
    if (inst.fabricObjects.length > 0) {
      $.each(inst.fabricObjects, function (index, fabricObj) {
        fabricObj.isDrawingMode = false;
          fabricObj.on('object:scaling', function(e) {
          var obj = e.target;
          var minWidth = 110, minHeight = 110;
  
          if (obj.getScaledWidth() < minWidth) {
            obj.scaleToWidth(minWidth);
          }
          if (obj.getScaledHeight() < minHeight) {
            obj.scaleToHeight(minHeight);
          }
  ...
  PDFAnnotate.prototype.loadFromJSON = async function(jsonData) {
      return new Promise((resolve, reject) => {
        ...
        const loadPromises = pages.map((page, index) => {
          return new Promise((resolvePage, rejectPage) => {
            inst.fabricObjects[index].loadFromJSON(page, () => {
              inst.fabricObjectsData[index] = inst.fabricObjects[index].toJSON();
              resolvePage();
            });
          });
        });
